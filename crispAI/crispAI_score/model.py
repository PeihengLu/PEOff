...
class ModelConfig:
    ...
    conv_seq_configs: list = field(
        default_factory=lambda: [(128, 3, 1), (32, 1, 1)]
    )  # (out_channels, kernel_size, stride)
    conv_pi_configs: list = field(
        default_factory=lambda: [(16, 4, 1)]
    )  # (out_channels, kernel_size, stride)
    ...
    output_dense: list = field(
        default_factory=lambda: [64, 3]
    )  # [concat_dim, ..., (mu, theta, pi)]
...
class CrispAI_pi(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i, (out_channels, kernel_size, stride) in enumerate(
            config.conv_seq_configs
        ):
            self.conv_seq_layers.append(
                nn.Conv1d(
            ...
            if config.conv_pool is not None:
                self.conv_seq_layers.append(
                    nn.MaxPool1d(
                        kernel_size=config.conv_pool[i][0],
        ...
        # lstm
        self.lstm = nn.LSTM(
            input_size=conv_seq_out_channels,
        ...
        for out_features in config.sequence_dense:
            self.sequence_dense.append(
                nn.Linear(in_features=temp_channels, out_features=out_features)
            )
        ...
        temp_channels = conv_pi_out_channels * (
            self.seq_len - config.conv_pi_configs[0][1] + 1
        )
        for out_features in config.pi_dense:
            self.pi_dense.append(
                nn.Linear(in_features=temp_channels, out_features=out_features)
            )
...
class CrispAI_pe(nn.Module):
    """
    updated version of the crispAI model to include longer sequence
    """
